...
- canvas element can have its size set in various ways,
  ...
  - stretch fractoin ie `height="2*"` - all the parents children have this type of dimension, and each one is a fraction of the total "_" - so if there were two children A and B with width(A) = 1_ and width(B) = 2\*, B would have 2/3rds the with of the parent and A would have 1/3rd
